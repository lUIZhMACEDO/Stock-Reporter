...
def get_sp500_tickers() -> list[str]:
    """Fetch S&P 500 symbols with primary + fallback sources for reliability."""
    headers = {"User-Agent": "Mozilla/5.0"}

    # Primary source: Wikipedia with browser-like headers.
    try:
        response = requests.get(SP500_WIKI_URL, headers=headers, timeout=REQUEST_TIMEOUT)
        response.raise_for_status()
        tables = pd.read_html(StringIO(response.text))
        for table in tables:
            if "Symbol" in table.columns:
                tickers = [symbol.replace(".", "-") for symbol in table["Symbol"].dropna().tolist()]
                if tickers:
                    return tickers
    except Exception:
        pass

    # Fallback source: DataHub CSV mirror for S&P 500 constituents.
    response = requests.get(SP500_FALLBACK_CSV_URL, headers=headers, timeout=REQUEST_TIMEOUT)
    response.raise_for_status()
    df = pd.read_csv(StringIO(response.text))
    if "Symbol" not in df.columns:
        raise RuntimeError("Unable to locate 'Symbol' column from fallback S&P 500 source.")

    tickers = [symbol.replace(".", "-") for symbol in df["Symbol"].dropna().tolist()]
    if not tickers:
        raise RuntimeError("No S&P 500 tickers were found from primary or fallback sources.")
    return tickers
